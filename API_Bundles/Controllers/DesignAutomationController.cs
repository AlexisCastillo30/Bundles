...
namespace designautomationbootcamp.Controllers
{
    ...
    public class DesignAutomationController : ControllerBase
    {
        ...
        public static string NickName { get { return InternalOAuth.GetAppSetting("APS_CLIENT_ID"); } }
        ...
        {
            dynamic oauth = await InternalOAuth.GetInternalAsync();
        ...
        {
            // input.data vendrá con JSON: { "drawingSheet": true, "threeD":true, ... "activityName":"ExportToPDFActivity", "browserConnectionId":"..." }
            JObject workItemData = JObject.Parse(input.data);

            bool drawingSheet = workItemData["drawingSheet"].Value<bool>();
            bool threeD = workItemData["threeD"].Value<bool>();
            bool detail = workItemData["detail"].Value<bool>();
            bool elevation = workItemData["elevation"].Value<bool>();
            bool floorPlan = workItemData["floorPlan"].Value<bool>();
            bool section = workItemData["section"].Value<bool>();
            bool rendering = workItemData["rendering"].Value<bool>();
            ...
            // Subir inputFile
            string inputFileNameOSS = $"{DateTime.Now:yyyyMMddhhmmss}_input_{Path.GetFileName(input.inputFile.FileName)}";
            string ossFileObjectId = await GetObjectId(bucketKey, inputFileNameOSS, oauth, fileSavePath);

            // Construir params.json
            dynamic inputJson = new JObject();
            inputJson.DrawingSheet = drawingSheet;
            inputJson.ThreeD = threeD;
            inputJson.Detail = detail;
            inputJson.Elevation = elevation;
            inputJson.FloorPlan = floorPlan;
            inputJson.Section = section;
            inputJson.Rendering = rendering;

            XrefTreeArgument inputFileArg = new XrefTreeArgument()
            {
                Url = ossFileObjectId,
                Headers = new Dictionary<string, string>() { { "Authorization", "Bearer " + oauth.access_token } }
            };
            XrefTreeArgument inputJsonArg = new XrefTreeArgument()
            {
                Url = "data:application/json," + ((JObject)inputJson).ToString(Formatting.None).Replace("\"", "'")
            };

            // Salida (PDF). El plugin generará "result.pdf" o "result0.pdf, etc."
            string outputFileNameOSS = $"{DateTime.Now:yyyyMMddhhmmss}_pdf_{Path.GetFileNameWithoutExtension(input.inputFile.FileName)}.pdf";
            XrefTreeArgument outputFileArg = new XrefTreeArgument()
            {
